...
export function AuthLesson({ lesson, title, subtitle, learn, code, tryIt, flow }) {
  const [tab, setTab] = useState(0);
  const content = [learn, code, tryIt, flow];
  return <Stack spacing={{ xs: 3, md: 4 }}><PageHeader eyebrow={`AUTH & SECURITY • LESSON ${lesson}`} title={title} subtitle={subtitle} /><Box sx={{ borderBottom: 1, borderColor: "divider", overflowX: "auto" }}><Tabs value={tab} onChange={(event, nextTab) => setTab(nextTab)} aria-label={`${title} lesson content`} variant="scrollable" allowScrollButtonsMobile>{tabs.map((label) => <Tab key={label} label={label} />)}</Tabs></Box>{content[tab]}</Stack>;
}

export function AuthExplorer({ title, subtitle, options }) {
...
export function AuthFlow({ title, steps }) {
